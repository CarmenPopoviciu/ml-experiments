...
export class GlobeDO implements DurableObject {
  private activeSessions: Array<Session>;
  ...
  async handleSession(webSocket, ip, latitude, longitude) {
    ...
    this.broadcast({ joined: { latitude, longitude }, activeSessions: this.activeSessions.map(session => {
      return { 
...
`)
      ...
      session.quit = true;
      this.activeSessions = this.activeSessions.filter(member => member !== session);

      // Broadcast to all other connections that this session has left.
